...
export default {
    template: /* html */`
        <div class="reader">
            ...
            <div s-elif="error" class="error">
                ...
                <div>{{$t('san-cli-ui-widget-reader.errors.' + error)}}</div>
            </div>
            <fragment s-else>
                <nav>
                    <article
                        ...
                        <div>
                            ...
                            <a href="{{item.link}}" target="_blank" rel="noopener noreferrer">
                                {{$t('san-cli-ui-widget-reader.origin')}}
                            </a>
                        </div>
                    </article>
                </nav>
    ...
    async fetchFeed(force = false) {
        ...
        try {
            const {results, errors} = await this.$callPluginAction('san-cli-ui-widget-reader.actions.fetch-news', {
                // 如果是知乎专栏的地址，就转换成对应的RSS，原理见https://rss.lilydjwg.me/
                url: this.data.get('data.config.url').replace(
                    /zhuanlan.zhihu.com|www.zhihu.com\/column/,
                    'rss.lilydjwg.me/zhihuzhuanlan'
                ),
                force
            });
